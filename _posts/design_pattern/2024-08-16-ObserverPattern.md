...
<br>
### 구독과 알람 기능을 생각해보자
유튜브의 Subscribe 기능이 바로 옵저버패턴 이다. 구독자들은 Observer 가 되어 유튜버를 구독하고 유튜버는 Subject 의 역할을 하며 이벤트가 생길때마다 구독자들에게 알람 (Notification) 을 날린다.  
...
강한 참조로 인한 메모리 Leak 에 대한 걱정보다는 오히려 목록상의 관찰자들을 순환하며 알람을 날리는 시점의 동기화(Sync) 로 인해 발생할 수 있는 딜레이를 더 신경 써야한다. Subject 의 이벤트 알람 기능에는 과도한 오버로드가 발생하지 않도록 주의하자.    
