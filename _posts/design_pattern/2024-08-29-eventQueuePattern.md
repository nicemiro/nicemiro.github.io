...
- Counting Semaphore 는 접근 가능한 스레드의 개수를 제한할 수 있으며 역시 raceCondition 이 발생할만한 병목지역에서의 대이터조작은 상호배제 원칙으로 작동. 대표적인 예시로는 스레드 풀을 사용하는 데이터베이스 트렌잭션, 프린터의 풀링시스템, 멀티 인스턴스를 사용하는 웹서버환경이 있음.
