...
특히나 AI 의 발전으로 작업시간이 줄어들었고 개발자 한명이 돌봐야할 서비스나 툴의 개수는 더 많아졌다.  
...
요즘은 더 쉽게 사용할 수 있는 스프링 부트나 Node.js 를 사용하는 등 더 쉽고 편리한 도구들이 많다.  
<br>

### 설치 라이브러리
대부분의 오류는 설치된 라이브러리 사이의 충돌로 인해 발생하는데 각각의 스프링 버젼에 맞는 라이브러리들의 호환 버젼을 검색해서 확인해야 한다.  
...
보통 root-context.xml 와 servlet-context.xml 둘을 나눠서 설정해준다.  
...
`DispatcherServlet` - 클라이언트가 브라우저에서 요청한 주소를 가지고 @Controller 에 작성된 매핑 주소 ( @GetMapping("/board/list") ) 와 연결한다. 실제 DispatcherServlet 은 요청 연결부터 ViewResolver 의 과정까지 모든 웹 처리의 모든 흐름을 관리한다.  
<br>
<br>
...
* `<bean class="InternalResourceViewResolver">`  
  ...
  WEB-INF 에 JSP 들을 모아두는 이유는 브라우저가 직접 전근할 수 없고 항상 Conroller를 거치게 하기 위함으로 
  반대로 resources 경로에 모아둔 css/js 파일들은 Spring Controller 를 거치지 않고 바로 파일이 반환됨  
...
* `<multipart-config>`  
  파일 업로드 경로, 파일 하나의 최대크기, 요청 전체 최대크기, 메모리 저장한계 설정. 초과시 업로드 실패함
