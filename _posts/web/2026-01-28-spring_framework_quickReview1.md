...
XML 만을 사용하는 초기 방식은 거의 사용되지 않거나 레거시 시스템에서만 유지되고 있고  
* 핵심 설정 XML + 빈 설정 @    
* 핵심 설정 @ +  빈 설정 @  
* 핵심 설정 자바 + 빈 설정 @   

이렇게 세 가지 타입으로 혼용되고 있다.  
XML 을 사용한 핵심 환경 설정 부분을 알아보자.  
<br>
...
보통 root-context.xml (어떤 이름이든 상관없음) , servlet-context.xml 둘을 나눠서 설정해준다.  
...
클라이언트 브라우져에서의 HTTP 요청은 톰캣(서블릿) 에 의해 전달될 서블릿이 결정된다.  
...
'annotation-driven' 으로 선언된 부분들은 @로 선언되는 스테레오 타입을 활성화해서 스프링 기동시 자동으로 Bean 객체를 만들어 등록한다.  
스프링 초기에는 xml 설정에서 사용할 객체들을 일일이 Bean 으로 선언해야 했다.  
...
* `<mvc:annotation-driven/>` MVC 기능의 어노테이션 활성화  
