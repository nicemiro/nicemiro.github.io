...
-- 테이블스페이스 생성권한 (CDB가 생성해도 되지만 밑에서 PDB_ADMIN 이 직접생성)
...
PDB_ADMIN 이 개발자 유저에게 수여할 권한들
```sql
-- MYPDB_ADMIN 이 개발자에게 수여할 DDL 롤
CREATE ROLE DEV_DDL_ROLE;
GRANT CREATE TABLE 
    ,CREATE VIEW
    ,CREATE SEQUENCE
    ,CREATE PROCEDURE TO DEV_ROLE;

-- MYPDB_ADMIN 이 개발자들에게 수여할 DML
